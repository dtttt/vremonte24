(function($){
    ...
    /* product.html - begin */
    $(document).ready(function() {
        $('.product-thumbs-slider').tinycarousel({
            axis: 'y',
            animationTime: 400,
            infinite: false
        });

        $('.products-spares-slider').owlCarousel({
            navigation: true,
            pagination: false,
            itemsCustom: [[1200, 6], [992, 5], [768, 4], [560, 3], [400, 2], [0, 1]],
            theme: "owl-theme owl-theme160"
        ...
        $('.product-thumbs .overview a').click(function(e) {
            if ($(this).parent().is('.active'))
            {
                e.preventDefault();
                return;
            }
            $('.product-big-img a').attr('href', $(this).attr('data-original-src'));
            $('.product-big-img img').attr('src','').attr('src', $(this).attr('href'));
            $('.product-thumbs .overview li.active').removeClass('active');
            $(this).parent().addClass('active');
            e.preventDefault();
        })

        $('.product-big-img a').fancybox({
            helpers: {
                overlay: {
                    locked: false
                }
        ...
        $('.product-tab').click(function() {
            ...
            $(this).addClass('active');
            $('.product-tab-content:visible').hide();
            $('.product-tab-content[data-tab="'+ $(this).attr('data-tab') +'"]').show();
        });

        $('.xs-product-imgs-slider').owlCarousel({
            pagination: true,
            itemsCustom: [[0,1]],
            theme: "owl-theme-pagination"
        });
